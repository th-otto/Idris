...
static const char version_str3[] = "    show0: Version 1.02  |\n";
static const char date_str[] = "       March 20, 1988    |\n";
static const char copyright_str[] = "  Copyright (C) 1987 by  |\n";
static const char cti_str1[] = "   (C)omputer            |\n";
static const char cti_str2[] = "     (T)ools             |\n";
static const char cti_str3[] = "       (I)nternational   |\n";
static const char reboot_msg[] = "<<-- the system must be rebooted for the changes to have any effect! -->>\n";

/*
 * hdp0, hdp1 and hdp2 aka
 * hd000, hd001, hd002
 * refer to the boot, swap and root partitions.
 * Regular partitions have numbers 3-6.
 * With "_all" appended, this includes the boot sector.
 * Partition 7 refers to the whole disk.
 */
static char root_device[] = "/dev/hdisk/hd007";
static char partition_name[] = "/dev/hdisk/hd003_all";
...
static char *ltoa_r(unsigned long val, char *buf)
{
	long remain;
...
static char *ltoa(unsigned long val)
{
	static char buf[30];
	ltoa_r(val, buf);
	return buf;
...
#ifndef __GNUC__
static void read_block0(int fd, const char *devname, void *buffer)
{
	if (fd < 0)
	{
...
static void read_rootsec(void *buffer)
{
#ifdef __GNUC__
	/*
	...
	strcpy(root_device, "/dev/hdisk/hd007");
	root_device[13] = '0' + harddisk;
	root_device[14] = '0' + lun;

	partition_header[25] = harddisk + '0';
	partition_header[26] = lun + '0';
	partition_header[38] = harddisk + '0';
	partition_header[39] = lun + '0';
	partition_header[51] = harddisk + '0';
	partition_header[52] = lun + '0';
	partition_header[64] = harddisk + '0';
	partition_header[65] = lun + '0';

	if ((fd = open(root_device, O_RDONLY)) < 0)
	{
		/*
		 * for compatibility with older kernel
		 */
		if (harddisk == 0 && lun == 0)
		{
			strcpy(root_device, "/dev/hdp7");
			fd = open(root_device, O_RDONLY);
			if (fd >= 0)
			{
				strncpy(&partition_header[25], "p3) ", 4);
				strncpy(&partition_header[38], "p4) ", 4);
				strncpy(&partition_header[51], "p5) ", 4);
				strncpy(&partition_header[64], "p6) ", 4);
			}
		}
	}
	read_block0(fd, root_device, buffer);
#endif
}


static void read_bootsec(int partition, void *buffer)
{
#ifdef __GNUC__
	/*
	 * fake bootsector for testing under linux
	 */
	...
	strcpy(partition_name, "/dev/hdisk/hd003_all");
	partition_name[13] = '0' + harddisk;
	partition_name[14] = '0' + lun;
	partition_name[15] = '3' + partition;
	if ((fd = open(root_device, O_RDONLY)) < 0)
	{
		/*
		 * for compatibility with older kernel
		...
		if (harddisk == 0 && lun == 0)
		{
			strcpy(partition_name, "/dev/hdp3_all");
			partition_name[8] = '3' + partition;
			fd = open(partition_name, O_RDONLY);
		}
	}
	read_block0(fd, partition_name, buffer);
#endif
}
...
static void set_reserved_sectors(int part_num, unsigned int count)
{
	struct bs bs;
	
	read_bootsec(part_num, &bs);
	/* in little-endian order */
	bs.res[0] = count & 0xff;
	bs.res[1] = (count >> 8) & 0xff;
	write_block0(partition_name, &bs);
}
...
static unsigned int print_reserved(const char *prefix, int part_num)
{
	struct bs bs;
	unsigned int reserved;
	
	if (!active_flag[part_num])
	{
		reserved = 0;
	} else
	{
...
static void show_info(struct rootsector *sector)
{
	...
	reserved[0] = print_reserved(" ", 0);
	reserved[1] = print_reserved(" | ", 1);
	reserved[2] = print_reserved(" | ", 2);
	reserved[3] = print_reserved(" | ", 3);
	print_str(" |\n");
...
int main(int argc, char **argv)
{
...

	{
		...
		if (harddrive != NULL)
		{
			harddisk = harddrive[0] - '0';
			lun = harddrive[1] - '0';
			if (harddisk < 0 || harddisk >= NUM_HARDDRIVES ||
				lun < 0 || lun >= NUM_LUNS ||
				harddrive[2] != '\0')
			{
	...
	if (argc >= 3)
	{
		...
		if (argc == 4)
			set_reserved_sectors(part_num, reserved);
	}
