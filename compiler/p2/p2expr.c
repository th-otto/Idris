...
BOOL adequ(ADDR *pl, ADDR *pr)
{
	if (pl->idx != pr->idx || pl->refs != pr->refs || pl->idx == XSP)
		...
		;
	else if (memcmp(pl->nm, pr->nm, LENNAME) == 0 && pl->bias == pr->bias)
		return TRUE;
	return FALSE;
}
 ...
 * fix to a volatile
 */
static BITS vfix(EXPR *p, BITS want, BITS set, BITS ty, BOOL wvol)
{
	BITS vwant;
	BITS got;

	if ((set = fix(p, want, set, ty)) == 0)
		return 0;
	else if (!wvol)
		return set;
	vwant = VOLSET;
 ...
 */
static BITS binop(EXPR *p, BITS want, BITS set, const unsigned char *tab)
{
	EXPR *l;
	EXPR *r;
	BITS lvol;
	...
	lvol = l->got & GVOL;
	rvol = r->got & GVOL;
	for (done = FALSE, i = LR; !done && i <= LTR; ++i)
	{
		l->got = lvol;
		r->got = rvol;
		switch (i)
		...
		case LR:
			...
			putfmt("LR:\n");
#endif
			if ((set = vfix(l, lwant, savset, lty, wvol)) == 0)
				done = FAIL;
			else if ((set = fix(r, rwant, set, rty)) != 0)
				done = SUC;
			break;
		case RL:
#ifdef DEBUG
			...
			if ((tab[0] & 0360) == 040)
				;
			else if ((set = fix(r, rwant, savset, rty)) == 0)
				done = FAIL;
			else if ((set = vfix(l, lwant, set, lty, wvol)) != 0)
				done = SUC;
			break;
		case LTR:
#ifdef DEBUG
			...
			if ((set = vfix(l, lwant, savset, lty, wvol)) == 0)
				panic("LTR bop");
			else if ((set = tempify(l, lwant, set)) != 0 && (set = fix(r, rwant, set, rty)) != 0)
				done = SUC;
			break;
		}
		if (done != SUC)
 ...
 */
static BITS getsop(EXPR *p, BITS want, BITS set, BITS pwant)
{
	EXPR *l = p->e.o.left;
	EXPR *r = p->e.o.right;
	BITS bwant;
	...
	if (!l->op)
	{
		cpyad(&l->f, &l->e.v);
		lgot = gotten(l);
		if (p->op != LGETS || !(want & WNULL) || !(lgot & WVMEM) || l->f.idx != X0)
		{
			pwant |= lgot & VOLSET;
		} else
		{
			if ((set = fix(r, WVMEM, set, l->f.ty)) == 0)
				return 0;
			cpyad(&p->f, &r->f);
			p->got = r->got & GVOL;
			return move(l, p, set);
	...
	if ((set = fix(l, pwant, set, l->e.o.ty)) == 0)
		return 0;
	savset = set;
	tab = pick(p, TRUE);
	ex.op = 0;
	...
	cpyad(&ex.e.v, &l->f);
	p->e.o.left = &ex;
	if ((!tab && (gotten(l) & (WPHL | WPBC))) || (set = binop(p, bwant, set, tab)) == 0)
	{
		if ((set = tempify(l, pwant, set ? set : savset)) == 0)
		{
			p->e.o.left = l;
			return 0;
		}
		...
		cpyad(&ex.e.v, &l->f);
		if ((set = binop(p, bwant, set, tab)) == 0)
		{
			p->e.o.left = l;
			return 0;
		}
	...
	if (set && !tab)
	{
		if ((p->f.idx == XSP || p->f.idx == XP0 || p->f.idx == XP1) &&
			(set = fgen(p, (xtor(l->f.idx) & BC) ? HL : BC, set, ptmov, FALSE)) == 0)
			return 0;
		if ((l->f.idx == XSP || l->f.idx == XP0 || l->f.idx == XP1) &&
			(set = fgen(l, (xtor(p->f.idx) & HL) ? BC : HL, set, ptmov, FALSE)) == 0)
			return 0;
		l->f.ty = l->e.o.ty;
		set = move(l, p, set);
	} else
 ...
 * do a post inc/dec
 */
static BITS postop(EXPR *p, BITS set)
{
	EXPR *l;
	EXPR *r;
	EXPR ex;
	...
	l = p->e.o.left;
	r = p->e.o.right;
	if ((set = fix(l, WPBC | WPHL | WMEM, set, l->e.o.ty)) == 0)
		return 0;
	cpyad(&p->f, &l->f);
	p->got = l->got;
	if ((set = force(p, (p->f.ty & 017) <= XUSHORT ? WSTACK : WVMEM, set, ty)) == 0)
		return 0;
	p->got &= ~(GZSET | GNSET);
...
BITS fix(EXPR *p, BITS want, BITS set, BITS ty)
{
	...
	case DINDIR:
	case DADDR:
		if ((set = fix(l, VOLSET | PTRSET | WTERM, set, l->e.o.ty)) != 0)
		{
			p->got = l->got & (GVOL | GJUNK);
			l->f.ty = p->e.o.ty;
			cpyad(&p->f, &l->f);
	...
	case DFNONLY:
		...
		break;
	case DELIST:
		if (fix(l, WNULL, set, l->e.o.ty) && (set = fix(r, want, set, r->e.o.ty)))
		{
			p->got = r->got;
			cpyad(&p->f, &r->f);
		} else
		...
		break;
	case DCAST:
		if ((set = fix(l, VOLSET, set, p->e.o.ty)) != 0)
		{
			p->got = l->got;
			cpyad(&p->f, &l->f);
			p->f.ty = p->e.o.ty;
 ...
 */
static BITS ftyp(EXPR *p, BITS want, BITS set, LEX ty)
{
	BITS reg;
	BITS junk;
	EXPR ex;
	...
	} else
	{
		...
		} else
		{
			...
			if (gotten(p) & (WSTACK | WPSTK) || p->f.idx == XHL)
				if ((set = fgen(p, BC, set, (p->f.nm[0] || p->f.bias) ? reref : intreg, FALSE)) == 0)
					return 0;
		}
		if (!reg)
			return 0;
...
BITS force(EXPR *p, BITS want, BITS set, LEX ty)
{
	...
	if ((set = ftyp(p, want, set, ty)) == 0)
		return 0;
	if (p->f.idx && (p->f.nm[0] || p->f.bias))
		if ((set = fgen(p, BC | HL, set, reref, FALSE)) == 0)
			return 0;
	if (1 < p->f.refs && xtor(p->f.idx) & REGSET)
		++p->f.refs;
	for (; 1 < p->f.refs; --p->f.refs)
		if ((set = fgen(p, BC | HL, set, deref, FALSE)) == 0)
			return 0;
	if (p->f.refs && (xtor(p->f.idx) & (R2 | R3 | R4 | AUTIDX)))
	{
		if ((set = fgen(p, BC | HL, set, intreg, FALSE)) == 0)
			return 0;
	} else if (!p->f.refs && p->f.nm[0])
	{
		if ((set = fgen(p, BC | HL, set, ptmov, FALSE)) == 0)
			return 0;
	}
	if (gotten(p) & want)
		return set;
	...
	for (; q->fwant; ++q)
	{
		...
		{
			if ((set = fgen(p, q->fset, set, q->ftab, !(q->fwant & PTRSET))) == 0)
				return 0;
			else
				break;
		}
...
#endif
	chsave = choff;
	if ((fix(r, WNULL, regset | (FNSET | TS), r->e.o.ty)) != (regset | (FNSET | TS)))
		panic("void");
	chput("`", NULL);
